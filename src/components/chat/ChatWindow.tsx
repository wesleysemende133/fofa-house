...
// Definição de tipo para mensagens para melhor tipagem TypeScript
interface Message {
  id: string;
  sender_id: string;
  receiver_id: string;
...
export default function ChatWindow({ propertyId, otherUserId }: ChatWindowProps) {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`chat:${propertyId}`)
      ...
      (payload) => {
        ...
        if (Number(newMsg.property_id) === Number(propertyId)) {
  ...
  // --- Handlers ---
  const handleSendMessage = async (e?: FormEvent, fileUrl?: string, fileName?: string) => {
  ...
  const handleFileUpload = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      await handleSendMessage(undefined, publicUrl, file.name);
    } catch (error) {
  ...
  const MessageBubble = ({ msg }: { msg: Message }) => {
    ...
    return (
      <div className={`flex ${isMe ? 'justify-end' : 'justify-start'} w-full`}>
        <div className={`
          relative max-w-[85%] sm:max-w-[75%] md:max-w-[70%] 
        ...
        `}>
             ...
             <div className="py-1">
               {isImage ? (
                 <img 
                  src={msg.file_url} 
                  alt="Attachment"
                  className="max-w-full rounded-lg h-auto max-h-60 object-cover cursor-pointer" 
                  onClick={() => window.open(msg.file_url!, '_blank')} 
                 />
               ) : (
                 <a href={msg.file_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 underline text-[14px]">
          ...
          <div className={`flex items-center justify-end mt-1 space-x-1 ${isMe ? 'text-orange-100/80' : 'text-gray-400'}`}>
            <span className="text-[10px] italic">{timeString}</span>
  ...
  return (
    <div className="flex flex-col h-full bg-[#FFF5F0] relative overflow-hidden">
      ...
      >
        ...
        <label className="p-1.5 cursor-pointer text-gray-400 hover:text-orange-500 active:scale-90" aria-label="Upload file">
          <Paperclip size={24} />
